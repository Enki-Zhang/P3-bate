    ...
    <el-drawer size="550px" class="drawer" :visible.sync="visible" :show-close="false" style="text-align: left"
        ...
        <header slot="title" class="header" v-else>
            ...
            <el-select v-if="isConditionNode()" v-model="properties.priority" size="mini" class="priority-select">
                <el-option v-for="item in priorityLength" :key="item" :value="item - 1"
                           :label="'优先级' + item"></el-option>
            </el-select>
        </header>
        ...
        <section class="condition-pane pd-10" v-if="value && isConditionNode()">
            <el-form size="medium" label-width="100px" label-position="top">
                <el-form-item label="条件说明" prop="condition">
                    <el-input v-model="conditionLabel" placeholder="条件说明" :maxlength="100"
            ...
            <el-tabs v-model="activeName" style="height: 100%" class="pane-tab">
                ...
                <el-tab-pane label="表单权限" name="formAuth">
                    <div class="form-auth-table">
                        <el-table :data="getFormOperates('f')" size="mini">
        ...
        <section class="approver-pane" style="height: 100%" v-if="value && isApproverNode()">
            <el-tabs v-model="activeName" class="pane-tab">
                <el-tab-pane :label="'设置' + (value.type === 'approver' ? '审批人' : '发起人')" name="config">
                    ...
                    <div v-else-if="value.type === 'approver'">
                        ...
                        <div style="border-bottom: 1px solid #e5e5e5; padding-bottom: 1rem">
                            <div v-if="approverForm.assigneeType === 'myself'" class="option-box"
                                 style="color: #a5a5a5">发起人自己将作为审批人处理审批单
                            </div>
                            <div v-else class="option-box">
                                ...
                                />
                                <el-form size="medium" label-width="100px" label-position="top">
                                    <el-form-item label="activiti表达式" prop="expression"
                                                  v-if="approverForm.assigneeType === 'input'">
                                        <el-input
                                    ...
                                    <el-form-item label="多人审批时采取的审批方式" prop="counterSign">
                                        ...
                                        <el-radio v-model="approverForm.optionalMultiUser" label="counterSign">会签
                                        </el-radio>
                                    </el-form-item>
                                </el-form>
                            </div>
                ...
                <el-tab-pane label="表单权限" name="formAuth">
                    <div class="form-auth-table">
                        <el-table :data="getFormOperates('s')" size="mini">
                            <el-table-column prop="vModel" label="字段名" align="left"></el-table-column>
                            <el-table-column prop="label" label="字段标题" align="left"></el-table-column>
                            <el-table-column prop="operate" label="操作" align="center" width="300px">
                                <template slot-scope="scope">
                ...
                <el-tab-pane :label="'设置抄送人'" name="config">
                    <div>
                        <div style="padding: 12px">
                            <el-radio-group v-model="approverForm.assigneeType" style="line-height: 32px"
                                            @change="resetOrgColl">
                                <el-radio v-for="item in copyAssigneeTypeOptions" :label="item.value" :key="item.value"
                                          class="radio-item">
                                    {{ item.label }}
                                </el-radio>
                            </el-radio-group>
...
<script>
    ...
    export default {
        ...
        methods: {
            getFormOperates() {
                let res = [];
                this.isApproverNode(true) && (res = this.approverForm.formOperates);
                this.isStartNode(true) && (res = this.startForm.formOperates);
                return res;
            },
            resetOrgColl() {
            ...
            initFormOperates(target) {
                ...
                // console.log(this.$store.state.designer.formItemList);
                // const formItems = this.$store.state.designer.formItemList.filter(t => t.cmpType !== 'custom');
                const formItems = this.$store.state.designer.formItemList;
                format(formItems);
                return res;
            },
            initCopyNodeData() {
            ...
            /** 判断是否是审批节点*/
            isApproverNode(is = false) {
                // if(is) console.log(this.value ? NodeUtils.isApproverNode(this.value) : false);
                return this.value ? NodeUtils.isApproverNode(this.value) : false;
            },

            isStartNode(is = false) {
                // if(is) console.log(this.value ? NodeUtils.isStartNode(this.value) : false);
