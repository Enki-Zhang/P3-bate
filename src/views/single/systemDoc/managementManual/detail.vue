<template>
  <el-row class="_root_page">
    <el-row class="page-default-pd page-default-h-has-breadcrumb">
      <el-row class="page-default-pd-bgc-white">
        ...
        <el-row class="block mg-b-10">
          ...
          >
            <el-row
              >文件名称：<span class="status-blue">{{
                detail.name
              }}</span></el-row
            >
            <el-row
              >文件编号：<span class="status-blue">{{
                detail.documentNo
              }}</span></el-row
            >
            <el-row
              >第 <span class="status-blue">{{ detail.version }}</span> 版
              <span class="status-blue">{{ detail.updateCount }}</span>
              次修订</el-row
            >
          </el-row>
          <iframe
          ...
          <el-row v-else type="flex" justify="center">
            <span
              class="status-red txt-i unable-select"
        ...
        <el-row class="block mg-t-10 mg-b-20">
          ...
          <el-row v-if="tbDataRelated.total" class="mg-t-20 mg-b-10 txt-c">
            <el-pagination
              :total="tbDataRelated.total"
              :current-page="tbDataRelated.current"
              :page-size="5"
        ...
        <el-row class="block mg-t-10 mg-b-20">
          ...
          <el-row v-if="tbDataForms.total" class="mg-t-20 mg-b-10 txt-c">
            <el-pagination
              :total="tbDataForms.total"
              :current-page="tbDataForms.current"
              :page-size="5"
              layout="total, prev, pager, next, jumper"
    ...
    <!-- 组件：相关文件选择 -->
    <dl-table-related
      v-model="dlTableRelatedVisible"
      :params="dlParams"
      @reloadTableData="getTableDataRelated()"
    ></dl-table-related>
    <!-- 组件：相关记录选择 -->
    <dl-table-forms
      v-model="dlTableFormsVisible"
      :params="dlParams"
      @reloadTableData="getTableDataForms()"
    ></dl-table-forms>
    <!-- 组件：自定义表单填写 -->
...
export default {
  ...
  beforeCreate() {
    // 更改当前路由面包屑 title
    if (this.$route.query.folderTitle !== undefined)
      this.man.bus.$emit(
        "changeCurrentRouteVirtualParentMetaTitle",
        JSON.parse(this.$route.query.folderTitle)
      );
  },
  created() {
  ...
  methods: {
    getDetail: function() {
      ...
      api.systemDocumentFindById(that.$route.query.id).then((res) => {
        if (res.data.status === 200) {
          ...
          that.previewSrc = !!res.data.data.filePath
            ? res.data.data.filePath
            : false;
          if (that.previewSrc !== false)
            that.previewSrc = that.mergePath(that.previewSrc);
    ...
    lookDetail: function(row) {
      ...
      if (
        !that.man.fast.inArray(
          "system-doc:management-manual:detail:detail",
    ...
    showApplyForm: function(row) {
      ...
      if (
        !that.man.fast.inArray(
          "system-doc:management-manual:detail:apply",
          that.userInfo.permissions
        )
      ) {
        that.$message.warning("您无权限进行此操作");
