  ...
  <el-row class="_root_page">
    <el-row class="page-default-pd page-default-h-has-breadcrumb">
        ...
        <el-col :span="8" :xs="8">
          <el-row class="block">
            <el-row type="flex" justify="space-between" class="title mg-tb-10">
              <span>待我审批</span>
              <img
                src="../../../assets/image/workbench/wddb.png"
                class="icon"
              />
            </el-row>
            <el-row
        ...
        <el-col :span="8" :xs="8">
          <el-row class="block">
            <el-row type="flex" justify="space-between" class="title mg-tb-10">
              <span>更新信息</span>
              <img
                src="../../../assets/image/workbench/gxxx.png"
                class="icon"
              />
            </el-row>
            <el-row
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12" :sm="24" :xs="24">
          <el-row class="table-has-title mg-t-20">
            <el-row type="flex" justify="space-between" class="title">
              <span
                >待我审批 (<span class="yellow">{{ tbDataPending.total }}</span
                >)</span
              >
                ...
                <el-table-column label="操作" width="80">
                  <template slot-scope="scope">
                    <el-link
                      type="primary"
                      :underline="false"
                      @click.native="showDLApprovalProcess(scope.row)"
                      >审批</el-link
                    >
                  </template>
                </el-table-column>
        ...
        <el-col :span="12" :sm="24" :xs="24">
          <el-row class="table-has-title mg-t-20">
            <el-row type="flex" justify="space-between" class="title">
              <span
                >更新信息 (<span class="purple">{{ tbDataUpdating.total }}</span
                >)</span
              >
              ...
              >
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    {{
                      `${scope.row.operation}${
                        scope.row.type === "CUSTOM_FORM" ? "表格" : "文件"
                      } ${scope.row.name}`
                    }}
                  </template>
                </el-table-column>
                <el-table-column prop="createTime" label="更新时间" sortable>
                  <template slot-scope="scope">{{
                    scope.row.operationDate
                      ? dayjs(scope.row.operationDate).format(
                          "YYYY-MM-DD HH:mm:ss"
                        )
                      : ""
                  }}</template>
                ...
                <el-table-column label="操作" width="80">
                  <template slot-scope="scope">
                    <el-link
                      v-if="
                        scope.row.operation !== '删除' &&
                          scope.row.operation !== '创建'
                      "
                      type="primary"
                      :underline="false"
                      @click.native="navUpdatingDetail(scope.row)"
                      >详情</el-link
                    >
                  </template>
                </el-table-column>
      ...
      <el-row class="table-has-title mg-t-20 processing">
        <el-row type="flex" justify="space-between" class="title">
          <span
            >办理中 (<span class="green">{{ tbDataProcessing.total }}</span
            >)</span
          >
              ...
              <template slot-scope="scope">
                <el-row type="flex" justify="space-around">
                  <el-link
                    type="primary"
                    :underline="false"
                    @click.native="showDLViewProcess(scope.row)"
                    >查看进度</el-link
                  >
                  <el-link
                    type="primary"
                    :underline="false"
                    @click.native="cancelProcess(scope.row)"
                    >删除</el-link
                  >
                </el-row>
              </template>
      ...
      <el-row class="table-has-title mg-t-20">
        <el-row type="flex" justify="space-between" class="title">
          <span
            >申请历史记录 (<span class="bule">{{ tbDataApply.total }}</span
            >)</span
          >
              ...
              <template slot-scope="scope">
                <el-row type="flex" justify="space-around">
                  <el-link
                    type="primary"
                    :underline="false"
                    @click.native="cancelProcess(scope.row, false)"
                    >删除</el-link
                  >
                </el-row>
              </template>
    ...
    <!-- 组件：待我审批 -->
    <dl-table-pending
      v-model="dlVisibleTablePending"
      :params="dlParams"
    ></dl-table-pending>
    <!-- 组件：更新信息 -->
    <dl-table-updating
      v-model="dlVisibleTableUpdating"
      :params="dlParams"
    ></dl-table-updating>
    <!-- 组件：办理中 -->
    <dl-table-processing
      v-model="dlVisibleTableProcessing"
      :params="dlParams"
    ></dl-table-processing>
    <!-- 组件：申请历史记录 -->
    <dl-table-apply
      v-model="dlVisibleTableApply"
      :params="dlParams"
    ></dl-table-apply>
    <!-- 组件：审批 -->
    <dl-approval-progress
      v-model="dlVisibleApprovalProcess"
      :params="dlParams"
      @done="getTableDataPending(5)"
    ></dl-approval-progress>
    <!-- 组件：查看进度 -->
    <dl-view-progress
      v-model="dlVisibleViewProgress"
      :params="dlParams"
    ></dl-view-progress>
...
._root_page {
  .statistics {
    .block {
      ...
      .title.yellow {
        border-left: 5px solid #e8b57b;
      }
      .title.red {
        border-left: 5px solid #f61010;
      }
